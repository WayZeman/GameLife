...
Проєкт орієнтований на **Neon (PostgreSQL)**. У [Neon Console](https://console.neon.tech) створи проєкт, скопіюй **Pooled** connection string у `DATABASE_URL` і **Direct** у `DIRECT_URL` (обидва з `sslmode=require`, якщо Neon їх додає). Для 6-значного коду входу можна задати **`AUTH_PIN_PEPPER`** (однаковий у `.env` і на **Vercel**). Якщо не задано, у продакшені використовується стабільний варіант на основі **`DATABASE_URL`** (після зміни рядка підключення до Neon старі коди перестануть підходити, доки не додаси явний `AUTH_PIN_PEPPER`). Після змін у `.env`: `npx prisma migrate deploy` (або `migrate dev` у розробці).

## Продакшн і SEO

- У **Vercel** (або іншому хості) задай **`NEXT_PUBLIC_SITE_URL`** на повний URL з `https://…` — для коректних Open Graph / `metadataBase`.
- У змінних оточення продакшену мають бути **Neon**, **OpenAI**, за бажанням **`AUTH_PIN_PEPPER`** (див. вище).
- Сторінка **404** (`not-found`) і метадані в `layout.tsx` уже налаштовані; перед продажем заміни бренд «GameLife» у `layout.tsx` / `README` на своє юридичне ім’я та додай політику конфіденційності окремим документом, якщо потрібно.
